...
@Component({
  selector: 'app-cwnfdetails',
  templateUrl: './cwnfdetails.component.html',
  styleUrls: ['./cwnfdetails.component.css']
})
export class CWNFDetailsComponent implements OnInit {
  Faculty:any;
  Courses:any;
  show:boolean=false;
  result:any;
...
this.result=[];
  ...
  CheckNoFaculty(){
    ...
    for (const i in this.Courses) {
      ...
      if(this.flag!=0){
        this.result.push(this.Courses[i].courseName);
